        ...
        <section class="content">
            ...
            <div class="box box-primary">
                <div class="box-header">
                    <h3 class="box-title">إعدادات عامة</h3>
                </div>
                <div class="box-body">
                    <form action="{{ route('dashboard.gallery.update') }}" method="POST">
                        ...
                        <input type="text" name="title" class="form-control"
                            value="{{ $settingGallery->title ?? '' }}">
                        <label>عنوان القسم (English)</label>

                        <input type="text" name="title_en" class="form-control"
                            value="{{ $settingGallery->title_en ?? '' }}">
                        ...
                        <label class="mt-3">الوصف (English)</label>
                        <textarea name="description_en" rows="3" class="form-control ckeditor">{{ $settingGallery->description_en ?? '' }}</textarea>
            ...
            <div class="box box-primary mt-4">
                ...
                <div class="box-body">
                    <form action="{{ route('dashboard.gallery.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <label class="mt-2">عنوان الصورة (English)</label>
            ...
            <div class="row">
                @foreach ($items as $img)
                    <div class="col-md-3 text-center mb-4">
                        <img src="{{ asset($img->image) }}" width="100%" height="180" style="object-fit:cover;"
                            class="rounded shadow">

                        <!-- نموذج تعديل الصورة والعنوان -->
                        <form action="{{ route('dashboard.gallery.editItem', $img->id) }}" method="POST"
                            ...
                            <input type="text" name="caption_en" value="{{ $img->caption_en }}"
                                class="form-control form-control-sm text-center mb-2">
