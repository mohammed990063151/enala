(function ($) {
  ...
  const wdtImageBoxWidgetHandler = function($scope) {
    ...
    if($scope.find('.wdt-image-box-holder').data('settings')) {
      ...
      // New condition
      if ($scope.find('.wdt-image-box-holder').hasClass('wdt-rc-template-custom-template-2')) {
        const $description = $scope.find('.wdt-content-description');
        const $button = $scope.find('.wdt-content-button.wdt-button-clone');
        const totalHeight = $description.outerHeight() + $button.outerHeight();
        $scope.find('.wdt-media-image-cover-container').css('--bottom-height', totalHeight + 'px');
      }
    }
  };
